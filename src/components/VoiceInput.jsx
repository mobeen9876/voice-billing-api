...
function VoiceInput({ onInvoiceGenerated }) {
  ...
  const transcribeAudio = async (audioBlob) => {
    ...
    try {
      ...
      else setError('Transcription failed: ' + data.message)
    } catch {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      const data = await res.json()
      if (data.success) { if (onInvoiceGenerated) onInvoiceGenerated(data) }
  ...
  return (
    <div className={styles.card}>
      ...
      <div className={styles.recordRow}>
        <button className={`${styles.recordBtn} ${isRecording ? styles.recording : ''}`}
          onClick={handleRecord} disabled={isTranscribing || isGenerating}>
      ...
      <div className={styles.textRow}>
        ...
        <div className={styles.textareaWrapper}>
          <textarea className={styles.textarea} value={text}
            ...
            rows={3} disabled={isTranscribing || isGenerating} />
          <button className={styles.clearBtn}
            onClick={() => { setText(''); setError('') }}
            disabled={isTranscribing || isGenerating}>Clear</button>
        </div>
      </div>
